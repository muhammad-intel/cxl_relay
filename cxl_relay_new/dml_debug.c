...
int simics_send_access_to_cosim(void *send_data,void *reply_data, int verbose, int time_start_flag, int time_end_flag)
{
   ...
   simics_print_fifo_pkt_data_and_log(rd, logfp,verbose,2);  
    ...
    printf("simics_send_access_to_cosim: packet_number=%d:  reply request end to end transaction time %lld milliseconds\n",spd->packet_number,time_recv_reply-time_send_req);
    fprintf(logfp,"simics_send_access_to_cosim: packet_number=%d:  reply request end to end transaction time %lld milliseconds\n",spd->packet_number,time_recv_reply-time_send_req);
    fflush(logfp);
   if(verbose)
   { 
	   fflush(logfp);
